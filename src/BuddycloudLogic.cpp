...
void CBuddycloudLogic::SendPresenceL() {
	...
	_LIT8(KPresenceStanza, "<presence><priority>10</priority><status></status><c xmlns='http://jabber.org/protocol/caps' node='http://buddycloud.com/caps' ver='s60-0.5.04'/><nick xmlns='http://jabber.org/protocol/nick'></nick></presence>\r\n");
	...
	for(TInt i = (iFollowingList->Count() - 1); i >= 0; i--) {
		...
		if(aItem && aItem->GetItemType() >= EItemRoster && aItem->GetId().Length() > 0) {
			...
			for(TInt x = 0; x < iGenericList->Count(); x++) {
				...
				if(aChannelItem->GetId().Compare(aSubscribedItem->GetId()) == 0) {	
...
//					aItemFound = true;
	...
	_LIT(KBetaTestersChannel, "/channel/beta_testers");
		...
		if(aIdLex.Val(aIdEnum) != KErrNotFound) {
			...
			if(aAttributeType.Compare(_L8("result")) == 0) {
				if(aIdEnum == EXmppIdRegistration) {
					...
					ShowInformationDialogL(R_LOCALIZED_STRING_NOTE_CREDENTIALS_SUCCESS);
				}
